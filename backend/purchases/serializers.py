...
class PurchaseOrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "paid_by", "paid_at", "created_at", "notes", "items",
        ]
        read_only_fields = [
            "reference", "status", "approved_by", "approved_at",
            "paid_by", "paid_at", "created_at", "created_by", "total_amount",
        ]
    ...
    def create(self, validated_data):
        ...
        for item_data in items_data:
            ...
            # unit_cost might be None here, which is now allowed by the model
...
class PurchaseMarkPaidItemSerializer(serializers.Serializer):
    ...
    new_price = serializers.DecimalField(
        max_digits=10, decimal_places=2, help_text="New price for the product (required)"
    )

class PurchaseMarkPaidSerializer(serializers.Serializer):
    items = PurchaseMarkPaidItemSerializer(many=True, help_text="List of purchase items with their new prices")