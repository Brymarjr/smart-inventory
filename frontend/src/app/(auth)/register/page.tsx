...
export default function RegisterPage() {
  ...
  async function onSubmit(values: RegisterValues) {
    ...
    try {
      ...
      if (data && typeof data === "object") {
        ...
        const serverMsg =
          data.detail || data.message || data.error || data.non_field_errors;
        if (serverMsg) {
  ...
  return (
    <div className="h-screen flex flex-col lg:flex-row bg-card overflow-hidden">
      <style jsx global>{`
        input::-ms-reveal,
        input::-ms-clear {
          display: none;
        ...
        <div className="relative z-10 space-y-10">
          ...
          <div className="space-y-4">
            ...
            <p className="text-slate-400 text-xl leading-relaxed max-w-sm font-light">
              Join Nigerian SMEs optimizing their operations with AI-driven
              inventory insights.
            </p>
          </div>
        </div>
        ...
        <Card className="w-full max-w-3xl border-none shadow-[0_20px_60px_rgba(0,0,0,0.12)] rounded-[2.5rem] p-4 lg:p-10 bg-card my-4">
                    ...
                    render={({ field }) => (
                      <FormItem className="lg:col-span-2 space-y-1">
                        ...
                        <FormDescription className="text-xs">
                          This will be used to generate your unique workspace
                          ID.
                        </FormDescription>
          ...
          <CardFooter className="flex justify-center border-t border-slate-50 mt-6 pt-6">
            <p className="text-base text-slate-500 font-semibold">
              ...
              <Link
                href="/login"
                className="text-[#2D31FA] font-black hover:text-[#1A1B4B] ml-1"
              >
                Log in
              </Link>
            </p>
