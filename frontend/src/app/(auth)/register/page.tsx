...
import { Loader2, Rocket, ArrowLeft, PackageSearch, Eye, EyeOff } from 'lucide-react';
  ...
  async function onSubmit(values: RegisterValues) {
    ...
    try {
      ...
      if (data && typeof data === 'object') {
        ...
        const fields: (keyof RegisterValues)[] = ['tenant_name', 'username', 'email', 'first_name', 'last_name', 'password'];
        ...
        const serverMsg = data.detail || data.message || data.error || data.non_field_errors;
        if (serverMsg) {
          const errorMessage = Array.isArray(serverMsg) ? serverMsg[0] : (typeof serverMsg === 'string' ? serverMsg : 'An unknown error occurred.');
          toast.error('Registration failed', {
      ...
      <div className="flex-1 flex items-center justify-center bg-slate-50 p-4 overflow-y-auto">
        <Card className="w-full max-w-3xl border-none shadow-[0_20px_60px_rgba(0,0,0,0.12)] rounded-[2.5rem] p-4 lg:p-10 bg-white my-4">
          <CardHeader className="text-center space-y-3 pb-6">
            ...
            <CardTitle className="text-4xl font-black text-[#2D31FA]">
              Register Organisation
            </CardTitle>
            <p className="text-slate-400 text-lg font-medium">
              Set up your administrative dashboard
            </p>
          </CardHeader>
                    ...
                    <FormItem className="space-y-1">
                      ...
                      <div className="relative">
                        ...
                        <button
                          ...
                          className="absolute right-5 top-1/2 -translate-y-1/2 flex items-center justify-center text-slate-400 hover:text-[#2D31FA] focus:outline-none transition-colors"
                        >
