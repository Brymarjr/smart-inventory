  ...
  return (
    <div className="space-y-6 p-6 relative">
      ...
      <div className="grid gap-6 md:grid-cols-1 lg:grid-cols-3">
        ...
        <Card className={`lg:col-span-1 border-l-4 transition-colors ${
          ...
          <CardContent className="grid gap-4 max-h-[600px] overflow-y-auto">
            {filteredAlerts.length === 0 ? (
              <div className="text-center py-8">
                <div className="bg-slate-100 text-slate-500 p-3 rounded-full inline-block mb-2">
                  <TrendingUp size={24} />
                </div>
              ...
              filteredAlerts.map((alert) => (
                ...
                >
                  ...
                  <div className="bg-slate-50 p-2 rounded mt-2 text-xs text-slate-700 border border-slate-100 group-hover:bg-blue-50 group-hover:border-blue-100 transition-colors">
                    {alert.description}
                  </div>
              ...
              <TableBody>
                {forecasts.map((item, idx) => (
                  <TableRow key={idx}>
                    ...
                    <TableCell>
                      <span className="font-bold text-blue-600 text-lg">{item.predicted_quantity.toFixed(0)}</span>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <ul className="space-y-3">
              {steps.map((step, i) => (
                <li key={i} className="flex items-start text-sm text-slate-700">
                  <span className="flex-shrink-0 flex items-center justify-center w-5 h-5 rounded-full border border-slate-300 text-[10px] text-slate-500 mr-3 mt-0.5">
                    {i + 1}
                  </span>
                  {step}
                </li>
        ...
        <div className="p-4 border-t bg-gray-50 flex justify-end gap-3">
          ...
          <Button 
            className={alert.anomaly_type === 'shrinkage' ? 'bg-purple-600 hover:bg-purple-700' : 'bg-blue-600 hover:bg-blue-700'}
            onClick={btnAction}
          >
