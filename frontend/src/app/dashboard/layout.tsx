...
export default function DashboardLayout({
  ...
  return (
    <div className="flex flex-col h-screen bg-background selection:bg-[#2D31FA] selection:text-white">
      ...
      <header className="h-16 bg-[#1A1B4B] text-white flex items-center justify-between px-4 z-30 shrink-0 shadow-md">
        <div className="flex items-center gap-4">
          <Button
            ...
            className="hidden md:flex text-white hover:bg-card/10 hover:text-white"
          >
          ...
          <div className="md:hidden">
            <Sheet open={isMobileOpen} onOpenChange={setIsMobileOpen}>
              ...
              <SheetContent
                side="left"
                className="w-64 p-0 bg-[#1A1B4B] border-white/10 text-white"
              >
                    ...
                    <Link
                      ...
                      className={`flex items-center px-3 py-2 text-sm font-bold uppercase tracking-widest rounded-md transition-colors ${
                        pathname === item.href
                          ...
                          : "text-slate-300 hover:bg-card/5 hover:text-white"
                      }`}
                    >
          ...
          <div className="flex gap-2 ml-4">
            {isSyncing && (
              <Badge
                variant="secondary"
                className="hidden lg:flex gap-1 bg-card/10 text-white border-none"
              >
            ...
            {showDelayedPending && (
              <Badge
                variant="destructive"
                className="hidden lg:flex gap-1 bg-red-500 hover:bg-red-600"
              >
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="[&_span]:text-[#1A1B4B] [&_div]:bg-card [&_div]:border-2 [&_div]:border-white [&_div]:shadow-sm">
            <UserNav />
          </div>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* --- DESKTOP SIDEBAR --- */}
        <aside
          className={`hidden md:flex flex-col bg-card border-r border-slate-200 transition-all duration-300 ease-in-out shrink-0 ${
          ...
          <div className="flex-1 overflow-y-auto py-6 px-3 space-y-1">
            {NAV_ITEMS.map((item) => {
              ...
              return (
                ...
                >
                  <item.icon
                    className={`h-5 w-5 shrink-0 ${isActive ? "text-[#2D31FA]" : "text-[#1A1B4B]"} ${isSidebarExpanded ? "mr-3" : "mr-0"}`}
                  />
                  {isSidebarExpanded && (
          ...
          <div className="p-4 border-t border-slate-100 bg-slate-50/50">
            ...
            >
              <Avatar className="h-9 w-9 border-2 border-white shadow-sm shrink-0">
                <AvatarImage
                  src={`https://ui-avatars.com/api/?name=${user.username}&background=1A1B4B&color=fff`}
                />
