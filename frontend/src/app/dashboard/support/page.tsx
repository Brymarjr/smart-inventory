        ...
        <DialogContent className="sm:max-w-[600px] h-[80vh] flex flex-col p-0">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-muted">
            ...
            {selectedTicket?.comments?.map((comment) => (
              <div
                key={comment.id}
                className={`flex flex-col ${comment.is_superuser ? "items-start" : "items-end"}`}
              >
                <span className="text-xs text-muted-foreground mb-1">
                  {comment.is_superuser ? "Support Team" : "You"} •{" "}
                  {new Date(comment.created_at).toLocaleString()}
                </span>
                <div
                  className={`p-3 rounded-lg text-sm max-w-[85%] shadow-sm ${
                    comment.is_superuser
                      ? "bg-card border text-slate-700"
          ...
          <div className="p-4 border-t bg-card">
            {selectedTicket?.status === "closed" ? (
              <div className="flex items-center justify-center gap-2 p-3 bg-slate-100 rounded text-muted-foreground text-sm">
