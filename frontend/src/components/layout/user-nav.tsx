...
import { toast } from "sonner"; // or "use-toast" from @/components/ui/use-toast
...
export function UserNav() {
  ...
  const handleLogout = async () => {
    try {
      await logout();
      // Using toast.success ensures a single, clean notification
      toast.success("Logout successful", {
        id: "logout-toast", // unique ID prevents duplicates
      });
      router.push("/login");
    } catch (error) {
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-56" align="end" forceMount>
        ...
        <DropdownMenuGroup>
          ...
          <SupportDialog>
            {/* Note: In shadcn/ui, putting a trigger inside an item often requires e.preventDefault() 
                on select to prevent the menu from closing prematurely. */}
