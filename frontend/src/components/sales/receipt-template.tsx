...
export const ReceiptTemplate = forwardRef<HTMLDivElement, ReceiptProps>(
  ({ sale, settings }, ref) => {
    ...
    return (
      <div ref={ref} className="w-[80mm] p-4 bg-white text-black font-mono text-sm leading-tight">
        {/* --- HEADER --- */}
        <div className="text-center mb-4">
          <h1 className="text-xl font-bold uppercase">{settings?.store_name || "Store Name"}</h1>
          <p className="text-xs mt-1 whitespace-pre-wrap">{settings?.store_address}</p>
        </div>
                        ...
                        return (
                            <tr key={index}>
                                ...
                                <td className="py-1 text-right">
                                    {lineTotal.toLocaleString(undefined, { minimumFractionDigits: 0 })}
                                </td>
                            </tr>
                        );
                    })}
        ...
        <div className="border-t border-dashed border-black pt-2 space-y-1">
            <div className="flex justify-between font-bold text-lg">
                ...
                <span>
                    ...
                    {Number(sale.total_amount).toLocaleString(undefined, { minimumFractionDigits: 0 })}
                </span>
            </div>
