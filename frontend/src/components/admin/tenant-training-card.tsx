...
export function TenantTrainingCard({ tenantId, tenantName }: TenantTrainingCardProps) {
  ...
  const fetchStatus = async () => {
    try {
      ...
      const { data } = await api.get(`/api/admin/train-models/${tenantId}/?t=${Date.now()}`);
      
      const serverDate = data.last_trained_at ? new Date(data.last_trained_at) : null;
      if (serverDate) {
  ...
  const handleTrain = async () => {
    ...
    try {
      ...
      pollInterval.current = setInterval(async () => {
        ...
        console.log(`🔄 Polling... Server Time: ${newTimeMs} vs Baseline: ${startTime}`);
        ...
        <div className="flex items-center justify-between">
            <div className="space-y-1">
                <CardTitle className="text-base flex items-center gap-2">
                    <BrainCircuit className={`h-5 w-5 ${isTraining ? 'text-blue-600 animate-pulse' : 'text-blue-600'}`} />
                    AI Model Status
                </CardTitle>
                <CardDescription>
                    {isTraining 
            ...
            <div className="flex items-center gap-3">
                {lastTrained && (
                    <div className="flex flex-col items-end text-xs text-muted-foreground">
                        <span className="flex items-center gap-1">
                            {status === 'complete' ? <CheckCircle2 className="h-3 w-3 text-green-600" /> : <Clock className="h-3 w-3" />}
                            {status === 'complete' ? "Updated Just Now" : `Last: ${formatDate(lastTrained)}`}
                        </span>
                    </div>
                )}
                ...
                <Button 
                    ...
                    className={`min-w-[140px] text-white transition-all ${
                        status === 'complete' ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'
                    }`}
                >
                    {isTraining ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Training...</> : 
                     status === 'complete' ? <><CheckCircle2 className="mr-2 h-4 w-4" /> Done</> : 
