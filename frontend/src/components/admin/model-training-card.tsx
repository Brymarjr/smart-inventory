  ...
  const handleTrain = async () => {
    ...
    try {
      ...
      toast.success("Training Complete", {
        description:
          "Demand forecasting models updated with latest sales data.",
      });
  ...
  return (
    <Card className="border-l-4 border-l-purple-500 shadow-sm">
      <CardHeader>
        ...
        <CardDescription>
          Manually trigger the Machine Learning engine to re-analyze sales
          patterns and update forecast accuracy.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="flex items-center justify-between bg-muted p-4 rounded-lg border">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              Status: <span className="text-green-600 font-medium">Idle</span> •
              Last run: Automatic (Daily)
            </p>
          </div>
        ...
        <div className="mt-4 flex items-start gap-2 text-xs text-amber-600 bg-amber-50 p-2 rounded">
          ...
          <p>
            Note: Training may slow down the server for 10-20 seconds.
