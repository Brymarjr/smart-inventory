...
export function IdleTimeoutWrapper({ children }: { children: React.ReactNode }) {
        ...
        <>
            ...
            {showWarning && (
                <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <div className="bg-white p-6 rounded-[2rem] shadow-2xl max-w-md w-full mx-4 text-center space-y-5 animate-in fade-in zoom-in-95 duration-200">
                        <div className="mx-auto bg-amber-100 p-4 rounded-full w-fit">
                            <AlertTriangle className="h-10 w-10 text-amber-600" />
                        </div>
                        <div className="space-y-2">
                            <h2 className="text-2xl font-black text-slate-900">Are you still there?</h2>
                            <p className="text-slate-500 font-medium">
                                For your security, you will be automatically logged out in <span className="text-red-600 font-black text-lg">{countdown}</span> seconds due to inactivity.
                            </p>
                        </div>
                        <div className="flex gap-3 justify-center pt-4">
                            <Button variant="outline" onClick={logout} className="w-full h-12 rounded-xl font-bold border-2">Log Out</Button>
                            <Button onClick={stayLoggedIn} className="w-full h-12 rounded-xl bg-[#2D31FA] hover:bg-[#1A1B4B] font-bold text-white transition-colors">I'm Still Here</Button>
                        </div>
                    </div>
                </div>
            )}
